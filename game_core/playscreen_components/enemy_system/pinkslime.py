"""
Pinkslime Enemy - A pinkslime enemy that uses A* pathfinding to navigate toward the player
"""
import pygame
import os
import heapq
...
class Pinkslime(Enemy):
    ...
    def __init__(self, x, y):
        """Initialize the Pinkslime enemy"""
        super().__init__(x, y, enemy_type="pinkslime")
    ...
    def _load_animations(self):
        """Load animation sprites for the pinkslime"""
        # Load pinkslime idle, hit, death, and run animations for all directions
        for direction in ["right", "left", "up", "down"]:
            self.load_animation("idle", direction, "Enemies_Sprites/Pinkslime_Sprites/pinkslime_idle_anim_all_dir")
            self.load_animation("hit", direction, "Enemies_Sprites/Pinkslime_Sprites/pinkslime_hit_anim_all_dir")
            self.load_animation("death", direction, "Enemies_Sprites/Pinkslime_Sprites/pinkslime_death_anim_all")
            self.load_animation("run", direction, "Enemies_Sprites/Pinkslime_Sprites/pinkslime_run_anim_anim_all_dir")
    ...
    def check_collision(self, collision_handler, tile_mapping, map_data):
        """Check if the pinkslime would collide with any solid tile corners
        ...
        # Create a rect for the pinkslime's next position
    ...
    def _move_towards_player(self, player_x, player_y, collision_handler=None, tile_mapping=None, map_data=None):
        """Move the pinkslime towards the player with collision avoidance and line of sight
        ...
        if distance_to_player <= proximity_threshold:
            ...
            # Set direction based on movement for animation
            # Pinkslime has 4 directions (up, down, left, right)
            if abs(dx) > abs(dy):
                # Horizontal movement is dominant
                self.direction = "right" if dx > 0 else "left"
            else:
    ...
    def update(self, player_x, player_y, collision_handler=None, tile_mapping=None, map_data=None):
        """Update pinkslime position and animation
    ...
    def update_animation(self):
        """Update the pinkslime's animation frame"""
    ...
    def draw(self, surface, camera_x=0, camera_y=0, zoom_factor=1.0):
        ...
        if zoom_factor != 1.0:
            ...
            # Draw the scaled pinkslime
            surface.blit(scaled_image, (draw_x, draw_y))
        else:
            # Draw the pinkslime at normal size
    ...
    def take_damage(self, damage_amount=10, knockback_x=0, knockback_y=0, collision_handler=None, tile_mapping=None, map_data=None):
        """Apply damage to the pinkslime and trigger hit animation
    ...
    def apply_knockback_to_player(self, player, **_):
        ...
        if self.rect.colliderect(player.rect):
            # Calculate knockback direction (away from pinkslime)
            ...
            if hasattr(player, 'take_damage'):
                player.take_damage(10)  # Pinkslime deals 10 damage

            # Apply slowing effect to the player (10% speed reduction for 1 second)
            if hasattr(player, 'apply_slowing_effect'):
                player.apply_slowing_effect(0.9, 60)  # 90% of normal speed (10% reduction) for 60 frames (1 second at 60fps)
